...
The script validates the stack identity and current health, preserves its Portainer-managed environment, replaces only the Compose revision and image digest, reasserts the false schedule gate, and asks Portainer to pull the image. Portainer 2.45 [updates Compose stacks asynchronously](https://github.com/portainer/portainer/blob/2.45.0/api/http/handler/stacks/stack_update.go): its successful update response means deployment has started, not that the new container is ready. The script waits through Deploying (status 3) and requires Active (status 1) plus an exact `ok` response from `/healthz`. A healthy old container while the stack is still deploying does not count as success. Each verification phase defaults to a two-minute budget, including its HTTP requests.

If the update fails or health verification expires, the script first establishes a settled Active, Inactive or Error state before sending one rollback request with the previous Compose file and environment. If deployment remains in progress, or the stack state cannot be established, it stops without a competing rollback and asks the operator to inspect Portainer. Rollback also waits through the asynchronous Deploying state; it is reported as verified only after Active and exact `ok`. A failed or unverified rollback remains a failed deployment.

Do not edit the Buntzen stack in Portainer while a deployment is running: workflow concurrency serializes workflow runs, but it cannot serialize an operator's direct Portainer edits. Portainer 2.45 rejects another update while the stack is Deploying; the deployment script respects that guard rather than trying to interrupt the active operation.
...
Before deploying schema changes, stop active work and make a consistent private backup of the appdata, including the database and its master key, plus the current Compose file, environment and image digest. This revision migrates to schema 6 for immediate booking timing, profile login URLs and ordered pass preferences. Restore matching appdata with the container stopped if rolling back to an older release; an older binary may start against newer additive columns while ignoring their behavior, so a healthy image-only rollback is insufficient evidence of compatibility.
