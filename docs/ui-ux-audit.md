...
| Home | Upcoming visits, shared Yodel sign-ins, recent jobs, and collapsible setup guidance |
| Lakes | Personal vehicle keyword, release schedule, pass preferences, and advanced booking URLs |
...
Account management is no longer an isolated half-width card above the global settings form. Yodel identities remain on Home. OTP and browser settings do not appear in lake preferences. Existing requests and queued jobs retain their saved configuration.
...
| Home setup instructions dominated normal use | Upcoming enabled visits ordered by date, recent jobs, shared sign-ins, and collapsible onboarding guidance |
